...
<head>
  ...
  <script>
    tailwind.config={theme:{extend:{colors:{peace:{50:'#f5f3ff',100:'#ede9fe',200:'#ddd6fe',300:'#c4b5fd',400:'#a78bfa',500:'#8b5cf6',600:'#7c3aed',700:'#6d28d9',800:'#5b21b6',900:'#4c1d95'}}}}}
  </script>
</head>
<body class="bg-gray-950 text-white min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <div class="hidden md:flex items-center gap-1">
          <a href="/" class="nav-link px-4 py-2 rounded-lg text-sm font-medium ${activePage==='home'?'text-purple-400':'text-gray-300'} hover:bg-white/5">Home</a>
          <a href="/affirmations" class="nav-link px-4 py-2 rounded-lg text-sm font-medium ${activePage==='affirmations'?'text-purple-400':'text-gray-300'} hover:bg-white/5"><i class="fas fa-sun mr-1 text-yellow-400"></i>Affirmations</a>
          <a href="/community" class="nav-link px-4 py-2 rounded-lg text-sm font-medium ${activePage==='community'?'text-purple-400':'text-gray-300'} hover:bg-white/5">Community</a>
          <a href="/stories" class="nav-link px-4 py-2 rounded-lg text-sm font-medium ${activePage==='stories'?'text-purple-400':'text-gray-300'} hover:bg-white/5">Stories</a>
          <a href="/milestones" class="nav-link px-4 py-2 rounded-lg text-sm font-medium ${activePage==='milestones'?'text-purple-400':'text-gray-300'} hover:bg-white/5">Milestones</a>
          <a href="/resources" class="nav-link px-4 py-2 rounded-lg text-sm font-medium ${activePage==='resources'?'text-purple-400':'text-gray-300'} hover:bg-white/5">Resources</a>
          <a href="/about" class="nav-link px-4 py-2 rounded-lg text-sm font-medium ${activePage==='about'?'text-purple-400':'text-gray-300'} hover:bg-white/5">About</a>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        <div class="md:col-span-2">
          ...
          <div class="flex items-center gap-4 mt-6">
            ...
            <a href="#" class="w-10 h-10 rounded-full glass flex items-center justify-center hover:bg-purple-600/30 transition-colors text-gray-400 hover:text-white"><i class="fab fa-instagram text-sm"></i></a>
            <a href="#" class="w-10 h-10 rounded-full glass flex items-center justify-center hover:bg-purple-600/30 transition-colors text-gray-400 hover:text-white"><i class="fab fa-youtube text-sm"></i></a>
          </div>
        </div>
        <div>
  ...
  <script>
    ...
    function showToast(message,type){const t=document.getElementById('toast');document.getElementById('toastMessage').textContent=message;t.style.background=type==='info'?'linear-gradient(135deg,#7c3aed,#4f46e5)':'linear-gradient(135deg,#059669,#065f46)';t.classList.add('show');setTimeout(()=>t.classList.remove('show'),3500);}
    async function likeItem(type,id,btn){try{const res=await fetch('/api/'+type+'/'+id+'/like',{method:'POST'});const data=await res.json();if(data.likes!==undefined){btn.querySelector('.like-count').textContent=data.likes;btn.classList.add('text-red-400');btn.classList.remove('text-gray-400');}}catch(e){}}
  </script>
</body>
</html>`
}
